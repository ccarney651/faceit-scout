"""First-run progress helpers (moved out of the retired native GUI). Pure text
formatting and a read-only DB check, testable off any UI."""
...
def test_setup_hint_walks_the_user_through_the_three_steps() -> None:
    assert setup_hint(False, False).startswith("Step 1 of 3")   # not calibrated
    assert setup_hint(True, False).startswith("Step 2 of 3")    # calibrated, no codes
    assert setup_hint(True, True).startswith("Step 3 of 3")     # ready to capture


def test_setup_hint_is_ascii_safe() -> None:
    for cal in (True, False):
        for codes in (True, False):
            setup_hint(cal, codes)   # must not raise building the string
...
def test_missing_faceit_db_reads_as_empty_without_creating_it(tmp_path) -> None:
    ...
    assert faceit_is_empty(missing) is True
...
def test_populated_faceit_db_is_not_empty(tmp_path) -> None:
    ...
    con.commit()
    con.close()
    assert faceit_is_empty(p) is False
...
def test_file_with_no_championships_reads_as_empty(tmp_path) -> None:
    ...
    con.commit()
    con.close()
    assert faceit_is_empty(p) is True


def test_position_is_always_shown() -> None:
    assert eta_text(1, 380, elapsed=2.0).startswith("match 1 of 380")
