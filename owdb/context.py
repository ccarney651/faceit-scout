...
This runs over the owdb connection with faceit ATTACHed read-only, so it can
join faceit rows against owdb's own ``map_instances`` in one query to report
...
def derive_code_context(db: Database, faceit_db_path: str, demo_code: str) -> CodeContext:
    ...
    # Cross-DB: has owdb already captured this map? (the ATTACH payoff)
    already = c.execute(
...
def format_context(ctx: CodeContext) -> str:
    """Human-readable stdout block for ``owdb code show``."""
