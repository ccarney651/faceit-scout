"""Command-line interface for owdb.

Only ``owdb calibrate`` exists so far (SPEC 13 step 1). Later subcommands
(``refs``, ``capture``, ``codes``, ``scout``, ``comps``, ``export``, ``review``)
land with their build-order steps.

DB path config mirrors faceit-sync (SPEC 3):
  own DB:    ``--db``        -> ``$OWDB_DB`` -> ``owdb.sqlite3``
...
log = logging.getLogger("owdb")


def _configure_logging(verbose: bool) -> None:
    level = logging.DEBUG if verbose else os.getenv("OWDB_LOG_LEVEL", "INFO").upper()
    logging.basicConfig(
        level=level,
...
def _db_path(args: argparse.Namespace) -> str:
    return args.db or os.getenv("OWDB_DB", "owdb.sqlite3")
...
def cmd_review(args: argparse.Namespace) -> int:
    ...
    print("\nResolve with:  owdb review --observation <id> --slot <i> --hero <name>")
    return 0
...
def cmd_drafts(args: argparse.Namespace) -> int:
    with Database(_db_path(args)) as db:
        ...
        print("\nFinalize with:  owdb drafts --finalize <map_id>"
              "   |   discard:  owdb drafts --discard <map_id>")
    return 0
...
def cmd_contribute_push(args: argparse.Namespace) -> int:
    ...
    token = args.token or os.getenv("OWDB_SYNC_TOKEN") or settings.get("sync.token", "")
...
def cmd_contribute_merge(args: argparse.Namespace) -> int:
    ...
    with connect_ro(_faceit_db_path(args)) as fdb:
        ...
        try:
            ...
            pstats = {}
    # No owdb DB needed: contributions declare their own custom heroes, so a
...
def cmd_refs_import(args: argparse.Namespace) -> int:
    ...
    print("Run `owdb doctor` to confirm coverage, then capture normally.")
    return 0
        ...
        try:
            ...
            if seen:
                ...
                line("OK" if not weak else "..",
                     ...
                     + (f"; {len(weak)} weak (see `owdb refs coverage`)" if weak else ""))
            if unseen:
...
def cmd_heroes_add(args: argparse.Namespace) -> int:
    ...
    print("Now learn its portrait:  open the app -> Learn heroes, or `owdb refs learn`.")
    return 0
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--db", default=None,
                   help="owdb SQLite path (default: $OWDB_DB or owdb.sqlite3)")
    ...
    rx.add_argument("--out", default="owdb_refs.zip")
    ...
    ri.add_argument("bundle", help="path to an owdb_refs.zip")
    ...
    cm.add_argument("--out", default="owdb_comps.json", help="payload to write")
