"""``owdb refs`` — the reference-portrait library (SPEC §6, build step 2).
...
def default_refs_dir(db_path: str) -> str:
    """Where ref crops are stored: a ``refs/`` dir next to the owdb DB."""
...
def run_refs_from_sheet(  # pragma: no cover - runtime-only path
    ...
    *,
    ...
    if profile is None:
        raise CaptureError(
            f"no calibrated profile for '{hud_variant}' — run `owdb calibrate` first")
    assert profile.id is not None
...
def run_refs_learn(  # pragma: no cover - runtime-only path
    ...
    *,
    ...
    win = "owdb refs learn — is this the right hero?"
    written = 0
