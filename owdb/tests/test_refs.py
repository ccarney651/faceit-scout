...
from owdb.faceit import connect_ro, load_heroes
from owdb.models import FaceitHero, HeroRef
from owdb.refs import find_close_pairs, find_missing, resolve_hero_name
...
def db(tmp_path: Path) -> Iterator[Database]:
    database = Database(str(tmp_path / "owdb.sqlite3"))
    yield database
    database.close()
...
def _profile(db: Database) -> int:
    from owdb.calibrate import build_profile
