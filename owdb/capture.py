...
log = logging.getLogger("owdb.capture")
...
def grab_frame(retries: int = 10, retry_delay: float = 0.05) -> tuple[Frame, int, int]:
    ...
    if frame is None:
        raise CaptureError(
            "no capture backend available — install owdb's capture extra "
    ...
    *,
    ...
    if require_division is not None and ctx.division != require_division:
        raise CaptureError(
            ...
            f"({ctx.championship_name}); owdb is set to {require_division} only. "
                ...
                if hits:
                    ...
                    if not warned_ban:
                        ...
                        log.error("Slot resolved to banned hero %s — ROI profile likely "
                                  "stale after patch. Run `owdb calibrate`.", names)
                        warned_ban = True
                    continue
    ...
    if over:
        raise CaptureError(
            ...
            f"exceeds {BAN_HIT_FAIL_RATE:.0%} — ROI profile is stale, run `owdb calibrate`")
    return counts
...
def run_hotkey_capture(  # pragma: no cover - runtime-only path
    ...
    *,
    ...
    crops_dir = crops_dir or default_crops_dir(getattr(db, "path", "owdb.sqlite3"))
...
def default_crops_dir(db_path: str) -> str:
    """Where per-slot capture crops live: a ``crops/`` dir next to the owdb DB."""
    from pathlib import Path
...
def _profile_and_refs(db: Any, cv2: Any, hud_variant: str) -> tuple[Any, Any]:  # pragma: no cover
    ...
    if profile is None:
        raise CaptureError(
            f"no calibrated profile for {w}x{h} '{hud_variant}' — run `owdb calibrate`")
    refs = db.get_refs(profile.id)
    if not refs:
        raise CaptureError(
            f"no refs for profile #{profile.id} — run `owdb refs capture` first")
    return profile, refs
