"""Typed records for owdb, plus JSON (de)serialisation for the pieces that
persist as opaque columns.

Pixel coordinates live in the DB, never in source (SPEC §5). Several of these
types are the in-memory shape of what gets written to owdb tables; the
...
class ObsDetail(NamedTuple):
    ...
    # (see owdb.analysis.classify_player_transition).
...
# ("... Master Central ...", "... Expert/Advanced/Open ..."). owdb defaults to
...
class CodeListing(NamedTuple):
    """A capturable demo_code with its context, for ``owdb codes list`` (SPEC §7)."""
...
class CodeContext:
    ...
    operator supplies six characters; owdb supplies the context."""
