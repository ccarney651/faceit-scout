...
input[type=range]{-webkit-appearance:none;appearance:none;border:0;padding:0;background:transparent;
  accent-color:var(--accent);height:22px;width:170px;cursor:pointer}
input[type=number]{width:66px;text-align:right}
.winlab{color:var(--fg);font-size:12.5px;font-weight:600;min-width:130px}
...
let SCOUT_N=null, META_WIN='20';   // SCOUT_N: number of recent matches, null = all
...
function renderScout(){
  ...
  const slider=el(`<input type="range" min="1" step="1">`);
  const num=el(`<input type="number" min="1" step="1">`);
  const lab=el(`<span class="winlab"></span>`);
  bar.append(slider,num,lab);
  const body=el(`<div></div>`);
  wrap.append(bar,body);

  let total=1;
  function syncControls(){
    total=Math.max(1,teamTotalMatches(SCOUT_TEAM));
    if(SCOUT_N==null || SCOUT_N>total) SCOUT_N=total;   // default & clamp: all matches
    slider.max=num.max=total; slider.value=num.value=SCOUT_N;
    lab.textContent = SCOUT_N>=total ? `all ${total} matches` : `last ${SCOUT_N} of ${total}`;
  }
  function render(){
    body.innerHTML='';
    body.appendChild(renderScoutBody(scoutData(SCOUT_TEAM, SCOUT_N>=total?null:SCOUT_N)));
  }
  function setN(v){ SCOUT_N=Math.max(1,Math.min(total,parseInt(v,10)||total)); syncControls(); render(); }
  sel.onchange=()=>{ SCOUT_TEAM=sel.value; SCOUT_N=null; syncControls(); render(); };
  slider.oninput=()=>setN(slider.value);
  num.oninput=()=>setN(num.value);
  syncControls(); render(); return wrap;
}
